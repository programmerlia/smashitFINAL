...
namespace Smash_IT.adminpage
{
    public partial class admin_manage_queue : System.Web.UI.Page
    {
        ...
        private void LoadRegisteredUsers()
        {
            using (SqlConnection c = new SqlConnection(connString))
            {
                SqlDataAdapter da = new SqlDataAdapter("SELECT UserID, Firstname + ' ' + Lastname AS DisplayName FROM tblPlayerAccount ORDER BY Firstname", c);
                DataTable dt = new DataTable(); da.Fill(dt);
                ddlRegisteredUsers.DataSource = dt; ddlRegisteredUsers.DataTextField = "DisplayName"; ddlRegisteredUsers.DataValueField = "UserID"; ddlRegisteredUsers.DataBind();
        ...
        protected void btnAddMember_Click(object sender, EventArgs e)
        {
            try
            {
                if (ddlRegisteredUsers.SelectedValue == "0") return;
                using (SqlConnection c = new SqlConnection(connString))
                {
                    c.Open();
                    int order = Convert.ToInt32(new SqlCommand($"SELECT ISNULL(MAX(OrderNo),0)+1 FROM tblEventParticipant WHERE EventID={ddlActiveEvents.SelectedValue}", c).ExecuteScalar());
                    new SqlCommand($"INSERT INTO tblEventParticipant (EventID, UserID, OrderNo, StatusName) VALUES ({ddlActiveEvents.SelectedValue}, {ddlRegisteredUsers.SelectedValue}, {order}, 'Waiting')", c).ExecuteNonQuery();
                }
                RefreshWorkspace(int.Parse(ddlActiveEvents.SelectedValue));
            }
            catch (Exception ex) { ShowAlert("error", "Error", ex.ToString()); }
        }
        ...
        protected void btnAddMassPlayers_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(txtMassPlayers.Text)) return;
                using (SqlConnection c = new SqlConnection(connString))
                {
                    c.Open();
                    foreach (string n in txtMassPlayers.Text.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries))
                    {
                        string[] parts = n.Trim().Split(' ');
                        if (parts.Length == 0 || string.IsNullOrWhiteSpace(parts[0])) continue;
                        SqlCommand cmdW = new SqlCommand("INSERT INTO tblPlayerWalkIn (Firstname, Lastname) VALUES (@F, @L); SELECT SCOPE_IDENTITY();", c);
                        cmdW.Parameters.AddWithValue("@F", parts[0]); cmdW.Parameters.AddWithValue("@L", parts.Length > 1 ? string.Join(" ", parts.Skip(1)) : "");
                        int wid = Convert.ToInt32(cmdW.ExecuteScalar());
                        int order = Convert.ToInt32(new SqlCommand($"SELECT ISNULL(MAX(OrderNo),0)+1 FROM tblEventParticipant WHERE EventID={ddlActiveEvents.SelectedValue}", c).ExecuteScalar());
                        new SqlCommand($"INSERT INTO tblEventParticipant (EventID, WalkInID, OrderNo, StatusName) VALUES ({ddlActiveEvents.SelectedValue}, {wid}, {order}, 'Waiting')", c).ExecuteNonQuery();
                    }
                }
                txtMassPlayers.Text = ""; RefreshWorkspace(int.Parse(ddlActiveEvents.SelectedValue));
            }
            catch (Exception ex) { ShowAlert("error", "Parse Error", ex.ToString()); }
        }
