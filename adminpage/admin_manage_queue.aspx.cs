...
namespace Smash_IT.adminpage
{
    public partial class admin_manage_queue : System.Web.UI.Page
    {
        ...
        private void RefreshWorkspace(int eventId)
        {
            try
            {
                using (SqlConnection c = new SqlConnection(connString))
                {
                    ...
                    DataTable dtCourts = new DataTable();
                    SqlCommand cmdEvtCourts = new SqlCommand("SELECT CourtID, CourtNumber FROM tblCourt WHERE CourtID IN (SELECT CourtID FROM tblEventCourtPool WHERE EventID=@EID)", c);
                    cmdEvtCourts.Parameters.AddWithValue("@EID", eventId);
                    new SqlDataAdapter(cmdEvtCourts).Fill(dtCourts);
        ...
        private void AdvanceTeam(int eventId, string matchType, string phase, int winP1, int winP2, SqlConnection c)
        {
            ...
            {
                SqlCommand cmdNextOrder = new SqlCommand("SELECT ISNULL(MAX(MatchOrder),0)+1 FROM tblMatch WHERE EventID=@EID", c);
                cmdNextOrder.Parameters.AddWithValue("@EID", eventId);
                int order = Convert.ToInt32(cmdNextOrder.ExecuteScalar());
                SqlCommand cmdNextCourt = new SqlCommand("SELECT TOP 1 CourtID FROM tblEventCourtPool WHERE EventID=@EID", c);
                cmdNextCourt.Parameters.AddWithValue("@EID", eventId);
                object courtObj = cmdNextCourt.ExecuteScalar();
        ...
        private void MergeToGrandFinals(int eventId, SqlConnection c)
        {
            ...
            if (activeMatches == 0)
            {
                ...
                SqlCommand cmdPend = new SqlCommand("SELECT MatchID, MatchType, Player1_ParticipantID, Player3_ParticipantID, BracketPhase FROM tblMatch WHERE EventID=@EID AND MatchStatus='Pending' AND Player2_ParticipantID IS NULL", c);
                cmdPend.Parameters.AddWithValue("@EID", eventId);
                new SqlDataAdapter(cmdPend).Fill(dtP);
                if (dtP.Rows.Count == 2)
                {
        ...
        protected void btnSaveManualMatch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection c = new SqlConnection(connString))
                {
                    ...
                    cmdOrder.Parameters.AddWithValue("@EID", eid);
        ...
        protected void btnExecuteDraft_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlConnection c = new SqlConnection(connString))
                {
                    ...
                    SqlCommand cmdCourts3 = new SqlCommand("SELECT CourtID FROM tblEventCourtPool WHERE EventID=@EID", c);
                    cmdCourts3.Parameters.AddWithValue("@EID", eid);
                    new SqlDataAdapter(cmdCourts3).Fill(courts);
                    ...
                    for (int i = 0; i < ids.Count; i += step)
                    {
                        ...
                        SqlCommand cmdInsDraft = new SqlCommand(@"INSERT INTO tblMatch (EventID, CourtID, MatchType, BracketPhase, Player1_ParticipantID, Player2_ParticipantID, Player3_ParticipantID, Player4_ParticipantID, MatchStatus, MatchOrder) VALUES (@EID, @CID, @FMT, @PH, @P1, @P2, @P3, @P4, 'Scheduled', @ORD)", c);
                        cmdInsDraft.Parameters.AddWithValue("@EID", eid);
                        cmdInsDraft.Parameters.AddWithValue("@CID", cid);
                        cmdInsDraft.Parameters.AddWithValue("@FMT", format);
                        cmdInsDraft.Parameters.AddWithValue("@PH", phase);
                        cmdInsDraft.Parameters.AddWithValue("@P1", p1);
                        cmdInsDraft.Parameters.AddWithValue("@P2", p2 == "NULL" ? (object)DBNull.Value : p2);
                        cmdInsDraft.Parameters.AddWithValue("@P3", p3 == "NULL" ? (object)DBNull.Value : p3);
                        cmdInsDraft.Parameters.AddWithValue("@P4", p4 == "NULL" ? (object)DBNull.Value : p4);
                        cmdInsDraft.Parameters.AddWithValue("@ORD", order);
                        cmdInsDraft.ExecuteNonQuery();
                        order++;
                    }
                }
