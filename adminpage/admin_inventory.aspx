...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <style>
        ...
        body { background-color: #f4f7fe; font-family: 'Poppins', sans-serif; color: var(--text-dark); }
        .inventory-wrapper { padding: 30px 40px; max-width: 100%; margin: 0 auto; }
        ...
        .admin-intro h1 { color: var(--smash-blue); font-weight: 800; margin: 0; font-size: 1.8rem; text-transform: uppercase; }
        
        .search-box {
            display: flex; background: #f8fafc; border: 1px solid #cbd5e1;
            border-radius: 8px; padding: 0 15px; height: 45px; align-items: center; width: 300px;
        }
        .search-input { border: none !important; outline: none !important; width: 100%; font-size: 0.9rem; background: transparent; font-family: inherit; color: var(--smash-blue); font-weight: 600; }

        /* ===== FILTER BAR ===== */
        .filter-bar { display: flex; gap: 10px; margin-bottom: 20px; align-items: center; }
        .btn-filter { padding: 8px 18px; border-radius: 8px; font-weight: 700; cursor: pointer; border: 1px solid #cbd5e1; background: #fff; color: #64748b; font-size: 0.75rem; text-transform: uppercase; transition: 0.2s; }
        .btn-filter.active { background: var(--smash-blue); color: white; border-color: var(--smash-blue); }

        /* ===== LAYOUT ===== */
        .inventory-layout { display: grid; grid-template-columns: 1.4fr 1fr; gap: 25px; }
        @media (max-width: 1200px) { .inventory-layout { grid-template-columns: 1fr; } }

        .card { background: #fff; border-radius: 12px; box-shadow: 0 4px 15px rgba(30, 58, 138, 0.05); overflow: hidden; border: 1px solid var(--border-color); }
        .card-header { background: var(--smash-blue); color: #fff; padding: 15px 20px; font-weight: 700; text-transform: uppercase; font-size: 0.9rem; letter-spacing: 1px; }

        /* ===== FORMS ===== */
        .input-panel { padding: 25px; background: #fff; border-bottom: 2px solid #f1f5f9; border-top: 5px solid var(--smash-blue); }
        .form-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 20px; margin-bottom: 20px; }
        .input-field { width: 100%; padding: 12px; border: 1px solid #e2e8f0; border-radius: 8px; font-family: inherit; font-size: 0.9rem; box-sizing: border-box; }
        .input-field:focus { border-color: var(--electric-yellow); outline: none; background-color: #fefce8; }

        .btn { padding: 12px 24px; border-radius: 8px; font-weight: 700; cursor: pointer; border: none; text-transform: uppercase; font-size: 0.8rem; transition: 0.2s; }
        .btn-add { background-color: var(--electric-yellow); color: var(--smash-blue); }
        .btn-save { background: var(--smash-blue); color: white; }
        .btn-cancel { background: #f1f5f9; color: #475569; }

        /* ===== TABLES ===== */
        .custom-grid { width: 100%; border-collapse: collapse; }
        .custom-grid th { background: #f8fafc; padding: 15px; text-align: left; font-size: 0.7rem; color: #64748b; text-transform: uppercase; border-bottom: 2px solid var(--border-color); }
        .custom-grid td { padding: 15px; border-bottom: 1px solid #f1f5f9; font-size: 0.9rem; }

        .pill { padding: 5px 12px; border-radius: 6px; font-size: 0.7rem; font-weight: 800; text-transform: uppercase; display: inline-block; }
        .pill-avail { background: #dcfce7; color: #166534; }
        .pill-rented { background: #fee2e2; color: #991b1b; }
        
        .action-link { color: var(--smash-blue); background: #eff6ff; padding: 6px 10px; border-radius: 6px; text-decoration: none; font-size: 0.9rem; }
        .action-link.delete { color: #e11d48; background: #fff1f2; }

        .status-msg { display: block; padding: 15px; margin-bottom: 20px; border-radius: 8px; font-weight: 600; }
        .msg-success { background: #f0fdf4; color: #166534; border-left: 5px solid #22c55e; }
        .msg-error { background: #fef2f2; color: #991b1b; border-left: 5px solid #e11d48; }
    </style>
</asp:Content>
