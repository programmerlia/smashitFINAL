    ...
    namespace Smash_IT.homepage
    {
        ...
        private sealed class RentalCartLine
            ...
            {
                public string ItemCategory { get; set; }   // Rental / Consumable
                public string EquipmentType { get; set; }
                public string EquipmentSpec { get; set; }
    ...
    WHERE ItemCategory = 'Consumable'
      ...
      AND ISNULL(EquipmentSpec,'') = @Spec;", con, tx))
            ...
            // =========================================================
            // ALLOCATION / INSERT OF RENTALS + CONSUMABLES
            // =========================================================
    ...
    VALUES (@UserID, @ReservationID, @ItemID, 0);", con, tx))
                        {
                            cmdIns.Parameters.AddWithValue("@UserID", Convert.ToInt32(Session["UserID"]));
    ...
    (@UserID, @ReservationID, @ModelID, @Quantity, @UnitPrice, 0, GETDATE());", con, tx))
            ...
            protected void btnSubmitReservation_Click(object sender, EventArgs e)
            {
                ...
                string rentalCartJson = (hfRentalCart.Value ?? "").Trim();
                string consumableCartJson = (hfConsumableCart.Value ?? "").Trim();
    ...
    (UserID, CourtID, ResDate, StartTime, EndTime, SportName, ReservationStatusName, IsPaid, PaymentStatus, RequiredAmount)
    VALUES
    (@UserID, @CourtID, @ResDate, @StartTime, @EndTime, @SportName, 'Pending', 0, 'Unpaid', @RequiredAmount);
    SELECT SCOPE_IDENTITY();", con, tx))
                            {
                                cmdInsRes.Parameters.AddWithValue("@UserID", userId);
                                cmdInsRes.Parameters.AddWithValue("@CourtID", courtId);
                                ...
                                cmdInsRes.Parameters.AddWithValue("@SportName", selectedSport);
                                cmdInsRes.Parameters.AddWithValue("@RequiredAmount", requiredAmountStoredPesos);

                                reservationId = Convert.ToInt32(cmdInsRes.ExecuteScalar());
                            }

                            CreateRentalRowsForReservation(con, tx, reservationId, resDate.Date, startTime, endTime, rentalLines);
                            CreateConsumableRowsForReservation(con, tx, reservationId, userId, consumableLines);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Response.Write("Reservation allocation error:<br/><pre>" + HttpUtility.HtmlEncode(ex.Message) + "</pre>");
                            return;
                        }
                    }
                }

                decimal paymongoChargeNowPesos = courtDepositPesos + rentalsFullPesos + consumablesFullPesos;
                int paymongoChargeNowCentavos = ToCentavos(paymongoChargeNowPesos);
                        ...
                        attributes = new
                        {
                            ...
                            line_items = new object[]
                            {
                                new
                                {
                                    ...
                                    description = "CourtID " + courtId + " • " +
                                                  resDate.ToString("yyyy-MM-dd") + " • " +
                            ...
                            metadata = new
                            {
                                ...
                                court_full = courtFullPesos.ToString("0.00", CultureInfo.InvariantCulture),
                                court_deposit = courtDepositPesos.ToString("0.00", CultureInfo.InvariantCulture),
                                rentals_full = rentalsFullPesos.ToString("0.00", CultureInfo.InvariantCulture),
                                consumables_full = consumablesFullPesos.ToString("0.00", CultureInfo.InvariantCulture),
                                paymongo_charge_now = paymongoChargeNowPesos.ToString("0.00", CultureInfo.InvariantCulture),
                                required_amount_stored = requiredAmountStoredPesos.ToString("0.00", CultureInfo.InvariantCulture)
                            }
                        }
                    }
                };
                ...
                catch (WebException ex)
                {
                    ...
                    {
                        File.WriteAllText(
                            HttpContext.Current.Server.MapPath("~/App_Data/paymongo_create_checkout_error.txt"),
    ...
    WHERE ReservationID = @RID;", con3))
            ...
            // FINALIZE PAYMENTS
            // =========================================================
            public static void FinalizePaymentsAfterPaymongoPaid(
                int reservationId,
                int userId,
                decimal rentalsFullPesos,
                decimal consumablesFullPesos,
                decimal courtDepositPesos)
            {
    ...
    VALUES ('Rental', @UserID, @RID, CAST(GETDATE() AS DATE), @Amt);", con, tx))
                            {
                                ...
                                cmd.Parameters.AddWithValue("@Amt", rentalsFullPesos);
                                cmd.ExecuteNonQuery();
                            }
    ...
    VALUES ('Consumable', @UserID, @RID, CAST(GETDATE() AS DATE), @Amt);", con, tx))
                            {
                                cmd.Parameters.AddWithValue("@UserID", userId);
                                cmd.Parameters.AddWithValue("@RID", reservationId);
                                cmd.Parameters.AddWithValue("@Amt", consumablesFullPesos);
                                cmd.ExecuteNonQuery();
                            }
    ...
    VALUES ('Reservation', @UserID, @RID, CAST(GETDATE() AS DATE), @Amt);", con, tx))
                        {
                            ...
                            cmd2.Parameters.AddWithValue("@Amt", courtDepositPesos);
