...
{
    public partial class q_queue : System.Web.UI.Page
    {
    ...
    SELECT 
        ...
        CASE 
            WHEN ISNULL(r.RequestStatus, '') = 'CancellationRequested' THEN 'CancellationRequested'
            ELSE r.ReservationStatusName
    ...
    SELECT 
        ...
        e.EventID AS ID,
        s.CourtID,
        CAST(s.StartTime AS TIME),
        CAST(s.ExpectedEndTime AS TIME),
        e.SportName,
        'Approved' AS Status,
        '' AS RequestStatus,
        'Approved' AS DisplayStatus,
        e.Title AS Name,
        0 AS IsWalkIn
    FROM tblEvent e
    JOIN tblCourtQueue q ON e.EventID = q.EventID
    JOIN tblActiveSession s ON q.QueueID = s.QueueID
    WHERE CAST(s.StartTime AS DATE) = CAST(@Date AS DATE)
      AND s.StatusName = 'Active'";
            ...
            for (int cNum = 1; cNum <= 6; cNum++)
            {
                ...
                for (int i = 0; i < slots.Count; i++)
                {
                    ...
                    {
                        overlappingRecords = overlappingRecords.Where(r =>
                            r.Field<string>("Status") != "Cancelled" &&
                            r.Field<string>("DisplayStatus") != "CancellationRequested"
                        ).ToList();
                    }
